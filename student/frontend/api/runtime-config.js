...
// script. Every page loads this BEFORE js/config.js.
//
// Set this in the Student Frontend Vercel project's Environment Variables:
//   ADMIN_STAFF_ORIGIN = https://<the-actual-admin-staff-deployment>.vercel.app
//
...
module.exports = (req, res) => {
  const adminStaffOrigin = (process.env.ADMIN_STAFF_ORIGIN || "").trim();

  res.setHeader("Content-Type", "application/javascript; charset=utf-8");
  // Cache briefly at the edge/browser so this cheap function isn't hit on
  // every single asset load, but short enough that updating the env var
  // and redeploying takes effect quickly.
  res.setHeader("Cache-Control", "public, max-age=60");

  res.status(200).send(
    `window.__ADMIN_STAFF_ORIGIN__ = ${JSON.stringify(adminStaffOrigin)};\n`
  );
};
